...
const calculateBillingAmounts = (quantity: number, rate: number): BillingCalculation => {
...
export const createBilling = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const {
      companyName,
      vehicleId,
    ...
    if (!companyName || !vehicleId || !recipientName || !recipientAddress || !workingTime || !rate) {
      const response: ApiResponse = {
        status: false,
    ...
    if (error.name === "ValidationError") {
      const response: ApiResponse = {
        status: false,
        message: (Object.values(error.errors)[0] as any)?.message || "Validation error",
        data: null,
      };
      res.status(400).json(response);
      return;
...
export const getBillings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
    const {
      searchQuery,
    ...
    if (isCompleted !== undefined) filters.isCompleted = String(isCompleted) === 'true';
    ...
    const response: ApiResponse<BillingPaginationResponse> = {
      status: true,
      message: result.bills.length > 0 ? "Billings retrieved successfully" : "No billings found",
      data: result,
    };
...
export const getBillingById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user?._id;
    ...
    const billing = await Billing.findOne({ _id: id, userId })
...
export const updateBilling = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (updateData.vehicleId) {
      const vehicle = await Vehicle.findOne({ _id: updateData.vehicleId, userId });
      if (!vehicle) {
        const response: ApiResponse = {
          status: false,
          message: "Vehicle not found or doesn't belong to user",
    ...
    if (cleanedData.companyName) cleanedData.companyName = cleanedData.companyName.trim();
    if (cleanedData.recipientName) cleanedData.recipientName = cleanedData.recipientName.trim();
    if (cleanedData.recipientAddress) cleanedData.recipientAddress = cleanedData.recipientAddress.trim();
    if (cleanedData.workingTime) cleanedData.workingTime = cleanedData.workingTime.trim();
    if (cleanedData.hsnCode) cleanedData.hsnCode = cleanedData.hsnCode.trim();
    if (cleanedData.billingDate) cleanedData.billingDate = new Date(cleanedData.billingDate);
...
// Delete a billing
export const deleteBilling = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user?._id;
    ...
    const billing = await Billing.findOneAndDelete({ _id: id, userId, isCompleted: true })
      .populate("vehicleId", "vehicleNumber");
...
export const getBillingStats = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
...
export const calculateBilling = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { quantity = 1, rate }: { quantity?: number; rate: number } = req.body;
